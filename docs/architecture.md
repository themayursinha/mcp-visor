...
internal/
  ...
  killswitch/                  Opt-in local stdio kinetic session stop (HMAC command + monotonic epoch)
  instructionauthority/        PCA island (H32): instruction continuity; optional authenticated request-bound adapter when settings.instruction_authority_continuity is true
...
intercepted tools/call
        ...
        ▼
 ┌──────────────────────────┐
 │ Kinetic session/epoch    │──Deny──▶ session/epoch revoked; one
 │ gate (opt-in)            │          terminal deny; no identity/H32/
 │                          │          redaction/policy/relay
 └──────┬───────────────────┘
  open  │
        ▼
 ┌──────────────────────────┐
 ...
 └──────┬───────────┘
        ▼
 ┌──────────────────┐
 │ H32 authenticated │──▶ Optional Ed25519 request-bound assertion;
 │ adapter (opt)     │     strip Visor key before redaction/relay
 └──────┬───────────┘
        ▼
 ┌──────────────────┐
 ...
 └──────┬───────────┘
  ok    │
        ▼
 Approval-bound calls recheck the kinetic session/epoch gate immediately
 before this commit, because approval releases runtimeMu during the wait.
        ▼
 ┌──────────────────┐
...
When `--kill-switch-dir` is empty, kinetic stop is compiled in but performs zero work: no directory I/O, no HMAC, no SIGKILL, and no `kinetic_stop_enforced` events. When enabled, `Run` fails closed before observability if StartupCheck finds a prior matching session/epoch revocation, an invalid command, or a missing/untrusted control dir. The monitor polls at 50ms (tests may shorten). An authenticated HMAC-SHA-256 command latches before later `tools/call`, SIGKILLs the supervised Unix process group, closes the session, persists monotonic state, and emits hash-chained `kinetic_stop_enforced`. The same session/epoch cannot restart. Remote `--server-url` plus an enabled dir is rejected before connect. Enforcement is not host-wide; it is not Wiz-fixed or CVE-fixed; detached descendants, kernel-uninterruptible work, privileged local tampering, and an H39 replica on another host remain residuals. Unix process-group containment uses `Setpgid` plus `kill(-pid, SIGKILL)`.

### 2.45 Kinetic session stop (`internal/killswitch/`, proxy gate)

Opt-in, default-off, local stdio emergency stop. Flags: `--kill-switch-dir DIR`, repeatable `--kill-switch-controller CONTROLLER_ID=KEY_FILE`, `--session-epoch N`. Empty dir disables the feature completely. Controller identity is a configured per-controller local HMAC-SHA-256 key plus the protected directory, not an MCP/model identity. H45 is a runaway-process stop for a cooperative same-UID stdio child: the launched MCP server shares the visor UID and can read the control directory and keys, so this is not an independent kill authority against a malicious sibling process. `mcp-visor stop --control-dir DIR --session-id ID --revoke-through-epoch N --controller-id ID --controller-key KEY_FILE --reason TEXT [--wait 3s]` publishes a signed command and optionally waits (default 3s, 0–30s) for matching durable `revoked_contained` state. CLI success after `--wait` means the hash-chained enforcement audit already committed; state is published only after that audit, and optional SIEM forwarding runs after state so a blocked export cannot delay acknowledgment. Latch, contain, audit, and state persist without waiting on `runtimeMu`; the WaitGroup `kineticOr` / `--wait` join also ends there, so leftover runtime-lock bookkeeping and optional SIEM forwarding cannot stall acknowledgment. The kinetic session/epoch gate is first on `tools/call`; approval-bound calls recheck immediately before the durable allow commit; `EncodeRaw` and supervised `Start` hold a dedicated I/O mutex across the revoked check and the write/launch so a latched stop cannot be followed by a successful encode or Start; contain closes pipes first (to unblock a stalled write) then acquires that mutex before kill. A valid stop kills only the launched Unix process group (`Setpgid` + `kill(-pid, SIGKILL)`; non-Unix kills the direct child) and closes this session. A lower stale epoch is ignored; the same or covered epoch cannot restart; a higher new epoch is a new authority generation. Enabled control-channel loss or corruption contains fail-closed (`contained_control_unavailable` / `contained_control_invalid`), including a `commands/` directory that disappears after monitor construction. The `kinetic_stop_enforced` audit event is hash-chained and synced after containment is issued. Polling is 50ms with a 250ms healthy-system operational bound, not a hard real-time guarantee. Remote `--server-url` is unsupported when enabled. Ordinary capability/policy revocation remains separate. Residual: an H39 replica on another host is unreachable; so are detached descendants that escape the supervised group, kernel-uninterruptible work, effects already committed, privileged local tampering, and a same-UID child that tampers with the control channel. No default-on, Wiz-fixed, CVE-fixed, host-wide, fleet, or universal containment claim.

### 2.5 Server Identity Attestation (`internal/serveridentity/`, proxy gate)

Optional, deterministic stdio server identity binding. For a server policy that pins `attestation: {kind: stdio_invocation_sha256_v1, digest: sha256:<64-hex>, entry_arg_positions: [0]}`, the proxy resolves the launched stdio invocation (`internal/serveridentity/identity.go`): the launcher command is resolved via PATH, symlinks are followed, and the regular executable file is streamed into SHA-256 over a versioned, injectively framed serialization (`stdio_invocation_sha256_v1`): a format-marker field, one field for the executable bytes, and one field per literal argv value carrying its ordinal index, each field prefixed by a component tag, a fixed-width big-endian index, and an explicit byte length (see the contract in `internal/serveridentity/identity.go`). Only the positions listed in `entry_arg_positions` (zero-based indexes into `ServerArgs`, excluding the executable) additionally bind the resolved local regular-file content at that position — `[0]` for `node server.js`, `python server.py`, or another local single-entry runner; multiple unique positions are allowed and are normalized by sorting a copy, so YAML list order is not part of identity. Undeclared file-valued args (logs, databases, datasets, output paths) are bound by their literal bytes only and are never opened or hashed, so mutable runtime data cannot change the identity. Binding the local invocation means two servers launched through the same local runner with different declared local payloads get different identities, closing the confused-deputy gap for shared launchers. Recognized canonical dynamic registry runners (`npx`, `uvx`, `bunx`, `pnpx`, `pnx`, `npm exec`, `npm x`, `yarn dlx`, `pnpm dlx`, `bun x`, `uv tool run`) are unpinnable: they select a package from a registry at runtime, so the literal spec cannot be content-bound and resolution fails closed. The classifier recognizes only exact canonical executable names and exact leading subcommand tuples; options-before-subcommand, renamed launchers, and shell wrappers are not inferred, and ordinary non-registry subcommands remain resolvable. Operators who need attestation must launch a locally installed/content-pinned executable or declare the local payload positions instead of a dynamic registry runner. The resolved digest is captured exactly once at proxy construction and is immutable for the proxy/stdio-child lifecycle; the same resolution shape (attestation kind plus normalized entry_arg_positions) is retained with it. Every `tools/call` applies the kinetic session/epoch gate first (when enabled), then compares the snapshot policy's pin against that cached identity before runtime limits, redaction, argument policy, taint, chains, approval, or relay. Mismatch or unresolved identity fails closed with one terminal deny audit event carrying identity fields and no arguments. Matching identity continues and emits `server_attested=true` on the terminal allow event. Policies without an attestation omit the verdict, preserve legacy behavior, and perform zero identity-resolution work: construction and reload never call the resolver for an unattested logical server. Attestation is restart-bound: hot reload never re-hashes launcher or payload paths. A pin introduced after an unattested start, a changed resolution shape, or a same-shape digest replacement compares against the cached startup identity and fails closed as unresolved/restart-required; removal may restore the explicitly requested unattested legacy path; a startup resolution failure stays unresolved and is not retried on reload. Approval terminal allow/deny events carry the complete policy plus identity snapshot captured before the runtime barrier was released. Tool descriptions, schemas, instructions, and handshake `serverInfo` are untrusted presentation data and never satisfy identity. This is local invocation pinning, not remote/hardware attestation.
...
- **25 enforced argument rule types**: `deny_path`, `allow_path`, `require_path_literal`, `allow_path_slot`, `allow_destination`, `allow_working_directory`, `deny_secret`, `allow_application`, `allow_skill`, `deny_permission_bypass`, `allow_activation`, `deny_command_pattern`, `allow_command_pattern`, `deny_command_keyword`, `deny_query_pattern`, `allow_query_pattern`, `deny_recipient_domain`, `allow_recipient_domain`, `allow_recipient`, `allow_resource_owner`, `allowed_repos`, `max_file_size`, `max_result_rows`, `max_export_rows`, `require_approval_always`
...
Package `instructionauthority` (H32) stays a proof island. When `settings.instruction_authority_continuity` is true, Visor requires a policy-pinned Ed25519 assertion over the exact canonical pre-redaction request, object/root, session, client, logical server, tool, expiry, and a single-use nonce, then calls `instructionauthority.Authorize` and strips `params._meta["mcp-visor/instruction-authority/v1"]` before later gates and relay. Unrelated metadata is preserved. The assertion issuer, not MCP metadata, authenticates the evaluation root. An unsigned self-asserted `TRUSTED_USER` envelope is denied. The setting defaults off (nil gate, zero behavioral delta). Reloads toggle the bool, keys, and function under the same runtime snapshot barrier as other policy-derived surfaces. Standard MCP has no provenance; omitting the envelope is denied only while the setting is on. This is not universal production prevention.
