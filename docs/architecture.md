...
Opt-in, default-off, local stdio emergency stop. Flags: `--kill-switch-dir DIR`, repeatable `--kill-switch-controller CONTROLLER_ID=KEY_FILE`, `--session-epoch N`. Empty dir disables the feature completely. Controller identity is a configured per-controller local HMAC-SHA-256 key plus the protected directory, not an MCP/model identity. `mcp-visor stop --control-dir DIR --session-id ID --revoke-through-epoch N --controller-id ID --controller-key KEY_FILE --reason TEXT [--wait 3s]` publishes a signed command and optionally waits (default 3s, 0–30s) for matching durable `revoked_contained` state. The kinetic session/epoch gate is first on `tools/call`; approval-bound calls recheck immediately before the durable allow commit. A valid stop kills only the launched Unix process group (`Setpgid` + `kill(-pid, SIGKILL)`; non-Unix kills the direct child) and closes this session. A lower stale epoch is ignored; the same or covered epoch cannot restart; a higher new epoch is a new authority generation. Enabled control-channel loss or corruption contains fail-closed (`contained_control_unavailable` / `contained_control_invalid`). The `kinetic_stop_enforced` audit event is hash-chained and synced after containment is issued. Polling is 50ms with a 250ms healthy-system operational bound, not a hard real-time guarantee. Remote `--server-url` is unsupported when enabled. Ordinary capability/policy revocation remains separate. Residual: an H39 replica on another host is unreachable; so are detached descendants that escape the supervised group, kernel-uninterruptible work, effects already committed, and privileged local tampering. No default-on, Wiz-fixed, CVE-fixed, host-wide, fleet, or universal containment claim.
