...
| `brake.approval_grants_total` | Holds resolved by human grant: (request hash, session) hold match consumed once, receipt map identifying human approve | `tool_call_allowed` / `approval_receipt_hash` + `request_hash` + `session_id` + `approval_receipt.decision`/`approver_id` | per log scope | computable today |
...
| `brake.unlogged_denials_total` | Declared terminal denials with no matching deny event on the same (session, hash) key; declarations missing either half report as unjoinable (mechanism proof; production use needs the join-key gap closed) | reconciliation of declared decisions vs `tool_call_denied` by (`session_id`, `request_hash`) | per log scope | needs new field |
