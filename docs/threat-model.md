...
H32–H43 (`internal/instructionauthority` through `internal/selfescalation`) prove specific PCA properties against caller-held evaluation roots. They do not sit on the `tools/call` intercept path. Do not cite those demos as evidence that Visor currently prevents instruction laundering, causal substitution, localhost-as-identity, symlink identity swap, a compromised policy store, a swarm exceeding its campaign budget, cross-principal ambient bleed, host-to-host grant relay, stale remembered grants, capability-composition laundering, cross-tool quota hopping, or self-granted meta-authority at the proxy.
