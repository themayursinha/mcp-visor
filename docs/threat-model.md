...
### PCA: optional H32 intercept, residual H33–H43 islands

H32 is optional at intercept: when `settings.instruction_authority_continuity` is true, Visor trusts a cooperating caller to hold and serialize `InstructionObject` plus `EvaluationRoot` at `params._meta["mcp-visor/instruction-authority/v1"]` outside model-controlled text and arguments. Standard MCP does not supply provenance. No HMAC, signature, or channel binding exists. A client that omits the envelope is not fixed in production; while the setting is off it is unprotected, and while the setting is on it is denied. H33–H43 (`internal/causalauthority` through `internal/selfescalation`) remain islands. Do not cite those demos as evidence that Visor currently prevents causal substitution, localhost-as-identity, symlink identity swap, a compromised policy store, a swarm exceeding its campaign budget, cross-principal ambient bleed, host-to-host grant relay, stale remembered grants, capability-composition laundering, cross-tool quota hopping, or self-granted meta-authority at the proxy.
