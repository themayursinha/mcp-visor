...
|--------|----------|------------|---------------------|
| Spoofed agent identity | Medium | Low | Identity-based policies match the operator-supplied `--client-id`; the core proxy does not authenticate it. `settings.require_verified_actor` fails closed without a process-start `VerifiedActorContext` delivered on `-verified-actor-fd` (typically fd 3 from AIP `visor-session`). MCP `tools/call` fields are never a source of that context. Hand-starting visor with a typed `--client-id` remains spoofable. [Agent Identity Plane](https://github.com/themayursinha/agent-identity-plane) produces the verified blob after STS+DPoP verification. |
...
| Threat | Severity | Likelihood | Control in mcp-visor |
|--------|----------|------------|---------------------|
| Agent denies making a tool call | Medium | Medium | Every terminal allow writes a standalone JSONL `tool_call_allowed` record (H14) and `Sync()`s it before relay (H19). Denies are also JSONL events but without that `fsync`. `--client-id` is operator-supplied, not authenticated by this binary. Cryptographic caller attribution is [Agent Identity Plane](https://github.com/themayursinha/agent-identity-plane), not visor. This is not a signed, complete repudiation control. |
...
### PCA: optional authenticated H32 assertions, residual H33–H43 islands

H32 is a proof island evaluator. The optional `settings.instruction_authority_continuity` path is an authenticated, request-bound assertion gate for cooperating deployments. A trusted issuer outside the MCP client/model authenticates the evaluation root and signs Ed25519 claims that bind the exact canonical pre-redaction request, object/root, session, client, logical server, tool, expiry, and a single-use nonce. Visor verifies that signature against policy-pinned public keys, then strips `params._meta["mcp-visor/instruction-authority/v1"]` before relay and preserves unrelated metadata. An unsigned self-asserted `TRUSTED_USER` envelope is denied. Standard MCP does not supply this assertion: while off there is no H32 intercept protection; while on unsigned or omitted assertions are denied. A compromised issuer/private key can still sign a false `TRUSTED_USER` root or a dangerous exact request. Theft of an unexpired assertion can be used first in the same session; the nonce stops second use, not that first-use race. Expiry and nonce replay resistance do not make this a general anti-prompt-injection system, and they do not prevent malicious content inside a legitimately authenticated USER-or-higher instruction, mistakes in the unchanged H32 lattice, H33–H43 classes, process-memory compromise, policy/key substitution, or compromise of the out-of-band session/key channel. This does not claim “Wiz fixed” or universal production prevention. H33–H43 (`internal/causalauthority` through `internal/selfescalation`) remain islands. Do not cite those demos as evidence that Visor currently prevents causal substitution, localhost-as-identity, symlink identity swap, a compromised policy store, a swarm exceeding its campaign budget, cross-principal ambient bleed, host-to-host grant relay, stale remembered grants, capability-composition laundering, cross-tool quota hopping, or self-granted meta-authority at the proxy.

### 18. Trajectory advisor is advisory novelty, not authorization

The default-off session trajectory advisor (`-trajectory-advisor`, `internal/trajectory`) is a bounded deterministic bigram heuristic over policy-reaching attempted `server:tool` transitions. Residual risks:

1. This is a novelty heuristic, not authorization, attack detection, compromise detection, intent inference, or proof of safety.
2. First transitions, warmup traffic, low-support sources, and previously seen malicious transitions can evade flags.
3. Benign upgrades, rare workflows, server renames, and tool renames can create false positives.
4. The state is process/session-local, in-memory, and lost on restart; there is no fleet baseline, persistence, tenant aggregation, or cross-session learning.
5. The heuristic sees exact server/tool names only. It does not inspect arguments, outputs, natural language, data flow, identities beyond the configured session proxy, or semantic similarity.
6. Denials before policy evaluation are absent from its trajectory. Calls reaching policy are counted even when later denied, so the trajectory is attempted behavior, not execution history.
7. A client can train or churn the bounded window. Window poisoning affects only advice and cannot create authority.
8. Audit advice inherits the existing audit-directory trust boundary. Prometheus remains optional operator telemetry.
9. No LLM hook ships. A future LLM advisor would require a separate contract and must remain optional, non-authorizing, unavailable-safe, redaction-bounded, and outside the Action path.
10. Advisor telemetry must not be represented as H33–H43 enforcement, and none of those proof islands is wired by this observer.

### Kinetic session stop (opt-in local stdio)

Default-off. Empty `--kill-switch-dir` means zero kill-switch behavior. Enabled only for a local stdio Visor session with `--kill-switch-dir`, `--kill-switch-controller CONTROLLER_ID=KEY_FILE` (repeatable), `--session-epoch`, explicit `--session-id`, and a durable `--audit-log`. `mcp-visor stop` publishes an HMAC-SHA-256 command into the protected control directory. Controller identity is that local per-controller key plus directory ownership/mode, not an MCP/model identity. H45 assumes a cooperative/non-malicious stdio server: the child shares the visor UID and can read control keys, so this is a runaway-process stop, not an independent kill authority. The kinetic gate is first in `tools/call`, with a second recheck after approval waits. `EncodeRaw` and supervised `Start` hold a dedicated I/O mutex across the revoked check and the write/launch; contain closes pipes first, then acquires that mutex before kill, so a latched stop cannot be followed by a successful encode or Start. A valid stop SIGKILLs only the launched process group and closes this session; it is not host-wide, fleet, SSH, or Palisade control. A lower stale epoch is ignored; the same/covered epoch cannot restart; a higher epoch is a new authority generation. Enabled control-channel loss or corruption contains fail closed, including a missing `commands/` directory after monitor construction. The `kinetic_stop_enforced` event is hash-chained and synced after containment is issued; `revoked_contained` state is published only after that audit, before optional SIEM forwarding. Latch, contain, audit, and state persist without waiting on the policy-reload lock. Polling is 50 ms with a 250 ms healthy-system operational bound and no hard real-time guarantee. Unix uses process-group containment; non-Unix kills the direct child only. Remote `--server-url` is unsupported when enabled. Ordinary capability/policy revocation remains separate. Residuals: an H39 replica on another host; detached descendants that escape the supervised Unix process group; kernel-uninterruptible work; effects already committed before observation; privileged local tampering of the control directory and keys; a same-UID child that tampers with that channel. This does not claim default-on, Wiz-fixed, CVE-fixed, host-wide, production-prevention, or universal containment.
