...
|--------|----------|------------|---------------------|
| Spoofed agent identity | Medium | Low | Identity-based policies match the operator-supplied `--client-id`; the core proxy does not authenticate it. [Agent Identity Plane](https://github.com/themayursinha/agent-identity-plane) can derive `--client-id` and `--session-id` from a verified actor chain (visor-gateway); that control is outside this binary. |
