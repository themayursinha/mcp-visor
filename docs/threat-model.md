...
### PCA: experimental H32 cooperating-client adapter, residual H33–H43 islands

H32 is a proof island. The optional `settings.instruction_authority_continuity` path is an experimental cooperating-client adapter, not an attacker-resistant `tools/call` control. The repository classifies the MCP client as untrusted, yet the client supplies `evaluation_root.origin.trust_class`. The proxy verifies consistency of that assertion, not the authority behind it; a compromised client can self-assert `TRUSTED_USER`. The envelope is not bound to the canonical request hash, server, tool, session, nonce, or expiry, so a `FILE_READ` assertion can accompany a different policy-allowed tool. The adapter does not strip the envelope; instruction content is forwarded unless argument redaction rewrites `params`, which drops extra members including the envelope. Standard MCP does not supply provenance. No HMAC, signature, or channel binding exists. A client that omits the envelope is not fixed in production. H33–H43 (`internal/causalauthority` through `internal/selfescalation`) remain islands. Do not cite those demos as evidence that Visor currently prevents causal substitution, localhost-as-identity, symlink identity swap, a compromised policy store, a swarm exceeding its campaign budget, cross-principal ambient bleed, host-to-host grant relay, stale remembered grants, capability-composition laundering, cross-tool quota hopping, or self-granted meta-authority at the proxy.
