...
H32–H42 (`internal/instructionauthority` through `internal/toolcorrelation`) prove specific PCA properties against caller-held evaluation roots. They do not sit on the `tools/call` intercept path. Do not cite those demos as evidence that Visor currently prevents instruction laundering, causal substitution, localhost-as-identity, symlink identity swap, a compromised policy store, a swarm exceeding its campaign budget, cross-principal ambient bleed, host-to-host grant relay, stale remembered grants, capability-composition laundering, or cross-tool quota hopping at the proxy.
