...
| H20 | A configured stdio server identity is resolved independently of MCP metadata: a locally resolved launcher executable and local payload file content are content-bound, while recognized dynamic registry launchers (`npx`, `uvx`, `npm exec`) are unpinnable and fail closed when attestation is configured; the identity is checked before argument policy or relay, and mismatch/unresolved denies with identity-bound audit evidence while a matching local identity passes | `TestServerIdentityMismatchDeniesBeforeArgumentPolicy`, `TestServerIdentityMatchAllowsToolCall`, `TestServerIdentityUnpinnableRegistryLauncherFailsClosed`, `TestServerIdentityUnpinnableRegistryLauncherRejected`, `TestServerIdentityNpmRunNotRejected`, `TestServerIdentityApprovalSigningFailureUsesPolicySnapshot`, `TestConfusedDeputyDemo` |
