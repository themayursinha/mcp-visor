...
| H20 | A configured stdio server identity is resolved independently of MCP metadata: a locally resolved launcher executable and local payload file content are content-bound, while recognized canonical dynamic registry runners (`npx`, `uvx`, `bunx`, `pnpx`, `pnx`, `npm exec`, `npm x`, `yarn dlx`, `pnpm dlx`, `bun x`, `uv tool run`) are unpinnable and fail closed when attestation is configured; only exact canonical executable names and exact leading subcommand tuples are recognized, so ordinary non-registry subcommands (`npm run`, `npm install`, `npm ci`, `yarn add`, `pnpm add`, `pnpm exec`, `bun run`, `bun add`, `uv run`, `uv tool install`) remain resolvable; the identity is checked before argument policy or relay, and mismatch/unresolved denies with identity-bound audit evidence while a matching local identity passes | `TestServerIdentityMismatchDeniesBeforeArgumentPolicy`, `TestServerIdentityMatchAllowsToolCall`, `TestServerIdentityUnpinnableRegistryLauncherFailsClosed`, `TestServerIdentityUnpinnableRegistryLauncherRejected`, `TestServerIdentityNpmRunNotRejected`, `TestServerIdentityDocumentedRegistryRunnersRejected`, `TestServerIdentityDocumentedRegistryRunnersFailClosed`, `TestServerIdentityNonRegistryPackageManagerSubcommandsNotRejected`, `TestServerIdentityApprovalSigningFailureUsesPolicySnapshot`, `TestConfusedDeputyDemo` |
