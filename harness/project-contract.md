...
5. **Audit selected security events** — Denies, approvals, chain detections, argument redactions, session taints, and session lifecycle emit structured events. Plain allows emit a standalone JSONL event that must be durably committed to the configured regular `O_SYNC` audit ledger **before** the `tools/call` is relayed downstream (H19 authorization-commit): a failed, short, non-durable, closed, poisoned, or **rotated/rebound** ledger append denies the call with zero relay on both transports. The hash chain is recovered from the exact O_SYNC append fd (fresh exclusive-create starts at genesis; an existing inode is recovered through the fd that will receive appends), and every commit validates live fd regularity plus that the configured path itself is a non-symlink regular directory entry referencing the constructor-bound inode (no-follow) before and after the write, so rotation while running fails closed. Every accepted constructor binding (fresh create or reopened existing inode) and both pre/post commit validations fsync the parent directory, so a same-inode hard-link rebind is accepted only through that directory-durability boundary; every observed commit-time symlink, including one resolving to the opened inode, fails with poison and zero relay; constructor opens reject a pre-existing symlink with kernel no-follow; any validation/sync/write failure poisons the logger, and the poisoned state blocks both authorization commits and ordinary Log writes; a constructor sync failure never pathname-removes a potentially foreign replacement. Output-only redaction has no standalone event beyond the terminal decision.
