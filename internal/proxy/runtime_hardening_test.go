...
func TestRuntimeLimitMaxArgumentSizeUsesOriginalBytes(t *testing.T) {
	p := New(Config{
		ServerName: "filesystem",
		Policy: mustLoadPolicy(t, `
version: "1.0"
...
`),
	...
	out := &bytes.Buffer{}
	client := mcp.NewParser(nil, out)
	raw := []byte(`{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"file_read","arguments":{"path":"/tmp/x"` + strings.Repeat(" ", 80) + `}}}` + "\n")
	_, action := p.interceptAndModify(raw, client)
	if action != "denied" {
		t.Fatalf("padded arguments must deny on original size, got %s", action)
	}
	if !strings.Contains(out.String(), "argument size") {
		t.Fatalf("expected argument-size denial, got %s", out.String())
	}
