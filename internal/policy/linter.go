...
func (res *LintResult) checkRule(path string, rule ArgRule, knownTypes map[string]bool) {
	...
	if rule.Type == "allow_resource_owner" {
		if len(rule.Patterns) == 0 {
			res.Violations = append(res.Violations, LintViolation{
				Path: path, Type: ViolationTypeWarning, Field: "patterns",
				Severity: SeverityWarning,
				Message:  "rule type 'allow_resource_owner' has no patterns",
			})
		}
	}

	if rule.Type == "allow_path_slot" {
		if len(rule.Patterns) == 0 {
			res.Violations = append(res.Violations, LintViolation{
				Path: path, Type: ViolationTypeWarning, Field: "patterns",
				Severity: SeverityWarning,
				Message:  "rule type 'allow_path_slot' has no patterns",
			})
		}
	}

	if rule.Type == "allow_destination" {
		if len(rule.Patterns) == 0 {
			res.Violations = append(res.Violations, LintViolation{
				Path: path, Type: ViolationTypeWarning, Field: "patterns",
				Severity: SeverityWarning,
	...
	if rule.Type == "allow_application" {
		if len(rule.Patterns) == 0 {
			res.Violations = append(res.Violations, LintViolation{
				Path: path, Type: ViolationTypeWarning, Field: "patterns",
				Severity: SeverityWarning,
				Message:  "rule type 'allow_application' has no patterns",
			})
		}
	}

	if rule.Type == "allow_skill" {
		if len(rule.Patterns) == 0 {
			res.Violations = append(res.Violations, LintViolation{
				Path: path, Type: ViolationTypeWarning, Field: "patterns",
				Severity: SeverityWarning,
