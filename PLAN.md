...
MCP is the first protocol boundary, not the final scope of the project. The deeper primitive is runtime authorization for autonomous agent actions:

```
actor + intent + action/tool + arguments + execution context
  -> deterministic policy decision
  -> allow / deny / redact / require approval
  -> signed receipt + audit trail
```

MCP Visor should dominate the MCP use case first, but the core policy, approval, receipt, redaction, and audit model should not be permanently coupled to MCP message shapes. Over time, the project can evolve into a general agent-action enforcement layer, with MCP as the flagship adapter.

Potential future adapters:

- MCP stdio and MCP HTTP/SSE
- OpenAI-style function/tool calls
- HTTP webhook action gateway
- CLI and shell execution wrappers
- GitHub Actions or CI automation gates
- Kubernetes admission or infrastructure-change gates
- Terraform, database, or cloud-admin operation gates

The invariant is that the enforcement path remains small, deterministic, fail-closed, and auditable. Protocol breadth must not expand the trusted computing base in the critical path.
...
Future architecture should separate the high-assurance enforcement data plane from the orchestration control plane.

`mcp-visor` remains the data plane:

- Parses protocol messages
- Evaluates deterministic policy
- Redacts sensitive data
- Tracks local chain context
- Verifies approval receipts
- Allows or denies execution
- Emits tamper-evident audit events

External systems such as n8n, Slack, Teams, SIEMs, Jira, PagerDuty, Vault, or KMS act as control-plane integrations:

- Route approvals to humans
- Enrich approval context
- Escalate when SLAs expire
- Forward evidence to audit systems
- Trigger signing through a narrow approval authority

n8n should not become the enforcement engine or the holder of final authority in the critical path. Its role is durable workflow orchestration around the visor. This preserves the small trusted computing base while still giving teams practical enterprise integrations.
...
### Phase 10: Agent Action Control Plane (v2)

**Goal:** Turn the current file/CLI approval workflow into a production-grade control-plane integration while keeping `mcp-visor` as the minimal deterministic enforcement data plane.

**Design principles:**

- Keep `mcp-visor` in the critical path and keep it deterministic.
- Treat n8n as an optional orchestration/control-plane adapter, not as the policy authority.
- Do not use an LLM to make allow/deny decisions. Semantic analysis can be advisory context for human approvers only.
- Prefer signed receipts over trusting callbacks by network location.
- Fail closed if approval verification, receipt validation, or callback authenticity is ambiguous.

**Deliverables:**

- [ ] HTTP/SSE transport for remote MCP servers
- [ ] Webhook event emitter for approval-required and audit events
- [ ] Signed approval callback verification
- [ ] Signed Decision Receipt format
- [ ] Hash-chained audit log entries
- [ ] Durable retry-based approval flow
- [ ] n8n workflow blueprint for Slack/Teams approval and SIEM forwarding
- [ ] Optional Vault Transit, KMS, or small approval-authority signer integration
- [ ] mTLS for visor-to-server and visor-to-control-plane connections
- [ ] SIEM export (syslog, Splunk, Elastic)

**Signed Decision Receipt fields:**

- Execution ID / nonce
- Session ID and client/agent identity
- Server and tool/action name
...
- Approver identity
- Decision (`allow`, `deny`, `approve`, `expired`)
- Expiry time
- Signature key ID
- Signature
...
| Mode | Behavior | Use case | Durability |
|---|---|---|---|
| Fast callback approval | `mcp-visor` holds the call briefly, emits a webhook, verifies a signed callback, then resumes the call | Local demos, small teams, short-lived approvals | Lost if client/session/proxy exits |
| Durable receipt approval | `mcp-visor` returns `approval_required` with an execution ID and request hash. The agent retries the same call with a signed receipt after approval | Production/containerized environments | Survives restarts and delayed approvals |

**n8n blueprint scope:**

- Receive `approval_required` webhook from `mcp-visor`
- Enrich context with optional Jira/PagerDuty/GitHub/SIEM lookups
- Send Slack/Teams interactive approval message
- On approval, request a signature from Vault Transit, KMS, or a tiny approval-authority service
- POST the signed decision receipt to the visor callback endpoint, or publish it for durable retry mode
- Forward final decision and receipt metadata to SIEM

**Success criteria:**

- A high-risk tool call cannot execute unless the visor verifies a valid receipt bound to the exact request hash.
- Replaying a receipt against modified arguments, a different tool, or an expired execution ID fails closed.
- n8n compromise alone cannot authorize execution without access to the signing authority.
- Audit output can prove who requested the action, which policy evaluated it, who approved it, and what exact payload was allowed.
- The existing file/CLI approval path remains available for local use and tests.
...
- [ ] v2: Signed decision receipts, webhook approvals, n8n control-plane blueprint, mTLS, SIEM export, HTTP transport
- [ ] v3: Durable adapter model beyond MCP, sandboxed tool execution (WASI/Wasmtime), Vault/KMS-backed approval signing
- [ ] v4: Deeper host-level enforcement, eBPF syscall telemetry, formal policy verification
