...
- Proof-carrying autonomy islands (H32–H37: instruction continuity, causal authority, principal derivation, resource identity, control-plane integrity, swarm authority budget) are isolated evaluators. They are not invoked on the proxy `tools/call` path. A passing island demo does not mean that class is enforced in production.
