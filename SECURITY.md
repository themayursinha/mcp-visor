...
- Notification-form `tools/call` is blocked on stdio and remote client paths, including the post-initialize handshake slot (no relay, no JSON-RPC response).
...
- H32 is an optional authenticated, request-bound assertion gate. `settings.instruction_authority_continuity` (default off) requires a policy-pinned Ed25519 assertion from a trusted issuer outside the MCP client/model. Assertions bind the exact canonical pre-redaction request, object/root, session, client, logical server, tool, expiry, and a single-use nonce. Visor strips `params._meta["mcp-visor/instruction-authority/v1"]` before relay and preserves unrelated metadata. An unsigned self-asserted `TRUSTED_USER` envelope is denied. Standard MCP clients omit this assertion: they are unprotected while off and denied while on. Share `--session-id`/`--client-id` with the issuer out of band. No private key enters Visor. This does not claim “Wiz fixed” or universal production prevention.
