...
## Optional authenticated H32 request-bound assertions

H32 remains a proof island evaluator. `settings.instruction_authority_continuity` (default off) enables an authenticated, request-bound assertion gate for cooperating deployments. A trusted issuer outside the MCP client/model signs an Ed25519 assertion over the exact canonical pre-redaction request, instruction object/root, session, client, logical server, tool, expiry, and a single-use nonce. Visor verifies that signature against policy-pinned public keys, then strips `params._meta["mcp-visor/instruction-authority/v1"]` before later gates and relay. Unrelated `_meta` is preserved. An unsigned self-asserted `TRUSTED_USER` envelope is denied. Standard MCP clients do not produce this assertion: they are unprotected while the setting is off and denied while it is on. Share a stable `--session-id` and `--client-id` with the issuer out of band. No private key enters Visor. This is not universal production prevention and does not claim “Wiz fixed.”
...
| Self-escalation invariant | H43 | `go run ./examples/self-escalation` |
